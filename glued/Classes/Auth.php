...
class Auth
{
    ...
    public function get_jwks(): array
    {
        ...
        if (!$hit) {
            ...
            if ($conf == []) {
                throw new AuthOidcException('Identity server connection failure, please reload this page.');
            }
            if ($conf['issuer'] != $oidc['uri']['realm']) {
                throw new AuthOidcException('Identity server configuration mismatch.');
            }
        ...
        if (!$hit) {
            ...
            if (!isset($jwks['keys'])) {
                throw new AuthOidcException('Identity server certificate mismatch.');
            }
    ...
    public function validate_jwt_token($accesstoken, $certs)
    {
        try {
            ...
            throw new AuthJwtException($e->getMessage(), $e->getCode(), $e);
        }
        return $r;
    }
    ...
    public function addrole(string $name, string $description): mixed
    {
        ...
        if ($res) {
            $this->events->emit('core.auth.role.created', $qp);
        }
        return $qp;
    }
    ...
    public function adddomain(string $name, string $primary_owner, $create_domain_if_none_exists = false): mixed
    {
        ...
        if ($create_domain_if_none_exists) {
            ...
            $q = $part_insert;
            if ($name == "") {
                $json['name'] = "Domain " . substr($domain_uuid, -4);
            } else {
                $json['name'] = $name;
            }
        }
